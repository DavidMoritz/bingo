...
const schema = a.schema({
  ...
  PhraseTemplate: a
    .model({
      id: a.id(),
      genre: a.string().required(), // e.g. "unicorns", "christmas", "tech-startup"
      phrases: a.string().array().required(), // 24-30 suggested phrases
      isAiGenerated: a.boolean().required().default(false), // true if generated by AI, false if manual
      usageCount: a.integer().required().default(0), // track popularity
      createdAt: a.datetime().required(),
      updatedAt: a.datetime().required(),
    })
    .identifier(['id'])
